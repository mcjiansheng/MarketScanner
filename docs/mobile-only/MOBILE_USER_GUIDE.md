...
扫描价签时使用固定框对准一个条码。相机持续自动对焦，Vision 最多 10 Hz；贴近导致条码略超框时会自动对同一帧尝试一次扩展范围。候选连续 2 帧锁定，通常采集 4 帧、最低 3 帧、最长 4 秒。若完整证据和可重算位置已经保存，但定位、深度或货架关联质量不足，应用会提示“低置信度已保存”，无需立即重复扫描。少于 3 帧可重算位置或 burst/exact node/raw pose 等权威证据不足时，条码仍以 `LOW_CONFIDENCE` 和空坐标保留，同时可生成独立补扫建议；只有身份串包、必需写入失败等无法安全界定或提交的会话级错误才终止。
