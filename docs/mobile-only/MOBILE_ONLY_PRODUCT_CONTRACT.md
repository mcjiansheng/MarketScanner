...
系统对每个可解析价签的主结果输出 `ACCEPTED` 或 `LOW_CONFIDENCE`；轨迹/会话层仍可输出 `UNAVAILABLE`。完整、exact-node 绑定且位置可重算、但质量不足的价签进入 `LOW_CONFIDENCE` 待复核，不自动创建补扫任务；权威证据缺失时仍保留 `LOW_CONFIDENCE` 条码占位行，并可另外创建非阻断 `RESCAN_REQUIRED` 补扫任务。补扫是建议，不是价签结果删除或整单失败。不允许静默猜测，不允许把低置信度算成自动通过，也不允许用户手工拖动修复。

### 2.3 Route A 冻结范围

Mobile V1 只承诺 `Fast reduced graph`，以及 Fast 质量失败后至多一次 `Full existing-graph optimization`。再次非 PASS 时，只要存在有限轨迹节点就必须输出不可发布的部分/诊断 Result，不得丢弃可读取数据；只有没有任何有限轨迹时才输出 `RESCAN_SESSION`。若 Full 求解器本身失败而 Fast outcome 已通过严格合同，则保留 Fast partial Result；ABI、身份、计数、内存或 quality 绑定不可信时仍必须终止。True sensor Deep（重新解码传感器并重建缺失视觉图证据）不属于 V1 功能，也不是本轮发布候选的隐藏 fallback。
...
- Debug 与 Release 必须生成同一 version 5 build identity 并进入同一真实扫描、存储、finalization、处理和结果实现；不得为 Debug 维护简化或模拟路径。`MobileBuildIdentity.canStartScan` 只要求身份完整且内部一致，不因 `build_configuration=debug` 或 `working_tree_state=dirty` 阻断测试。Debug 可使用 dirty tracked tree，但必须记录 `source_ref`、唯一 tracked patch SHA-256 和 `production_eligible=false`；Release 继续拒绝任意 dirty tracked tree，并仅在 clean 时记录 `production_eligible=true`。默认 `RTABMapApp` Run 与 `RTABMapApp-QualifiedDevice` 仍使用 Release，以保留真实性能与现场资格含义。
- 结果完整性与生产资格是两个字段：只要图、坐标、价签、framing、身份和原子提交等真实质量门通过，Debug 允许使用明确标记为 `CALIBRATION_PENDING` 的初值生成 `result_scope=TEST`、`result_quality_status=COMPLETE`、`publish_permitted=true` 的完整测试成果，手机和 PC 必须保留全部表格、地图、价签和审计文件；同时固定 `production_publish_permitted=false` 和 `test_calibration_override_applied=true`。clean Release 只有在标定状态为 `CALIBRATED/NOT_APPLICABLE` 且其余门全部通过时，才能设置 `result_scope=PRODUCTION` 与 `production_publish_permitted=true`。测试放行不得绕过损坏输入、身份串包、低置信价签、坐标缺失、穿架、图质量失败或不可原子提交等真实完整性门。
...
- Final trajectory：优化节点 → 1 Hz 重采样（ceil/floor UTC 秒，XY 线性、yaw 最短角、uncertainty 保守上界），跨 lost / disconnected / floor change / 超大间隔 / 时钟不连续输出 `UNAVAILABLE`。不同 component 的 native 输出不要求全局时间单调，只要求 component 内单调；业务重采样前排序且禁止跨 component 插值。
- 非 PASS/无 publish node：有 `initialMapPose` 时保留起点所在 component 的 `DEGRADED_MAP_ALIGNED` prior-map 诊断坐标；没有安全 gauge 时只保留确定性主 component 并写入 `local_x_m/local_y_m/local_yaw_deg`，地图列为空。两类结果均 `publish_permitted=false`。
...
- 价签 observation 只有属于 verified complete burst v2，且与 burst frame 构成 exact observation/frame 一对一关系时才可进入自动通过路径；解析全程 true streaming。共享 64 KiB JSONL reader 对每行 caller body 建立独立 autorelease pool，避免 Foundation 临时对象在 200k 规模长时进程中累积；保留值仍按正常强引用生存，UTF-8/duplicate-key/nesting 严格性不得放宽或重复解析。schema v2 还必须精确绑定源数据库 node ID/stamp/map ID，并保存 `RTABMAP_BOUND_NODE_LOCAL` 的 `point_in_bound_node_frame`；最终定位只执行 `P_final = T_final_node * P_node`，不再按 5 秒窗口猜节点，也不再重复应用 prior-map gauge。历史 v1 价签保留业务身份但清空可发布坐标并生成 rescan。多帧证据融合为物理实例；货架结果同时输出 `shelf_segment_id`、`distance_from_shelf_start_cm` 与 `position_ratio`。单条坏 burst/observation 或少量未消费 frame 只降级相应价签：≥3 帧且 node-local 权威完整时可保留低置信度位置，否则保留条码和空坐标；需要时另附 `RESCAN_REQUIRED` task。
- 手机结果使用 coordinate contract v2：`COMPLETE/publish_permitted` 必须同时满足 prior-map 坐标权威、图质量 PASS、零 degradation/legacy/LOW_CONFIDENCE/unpositioned/unassociated/rescan；result library 和 task recovery 必须二次验证，不能只信 writer 当时给出的状态字符串。
- 价签结果质量门为 `ACCEPTED / LOW_CONFIDENCE`，补扫任务独立使用 `RESCAN_REQUIRED`。recovering/weak、低 depth/view、node uncertainty、位置离散、无可靠货架候选、burst/graph/exact node/node-local point/measurement method/位置权威不足都不得删除 PriceTags；可恢复多少输出多少并标记 `LOW_CONFIDENCE`。legacy v1 coordinate frame 必须清空可发布坐标并补扫。身份串包、不可界定文件、重复 durable ID、不可证明水位和数据库/结果提交损坏仍是会话级 fatal，且必须保留原始会话。
- `scan_events.jsonl`、clock、burst、trace 和其他证据均来自不可变 snapshot。文件不可读/超限、缺 final newline、身份串包、不可证明的水位线、formal state 矛盾仍 fail closed；可隔离的记录级 schema/node-binding 错误进入 degradation audit，不得抹掉其他结果。scan event 的 `trackingSessionId` 必须逐行与当前处理 session 精确一致；当前 finalized metadata 尚未定义 scan-event count/last-ID，因此不得把 strict identity/framing 表述成 exact cardinality watermark。trace parser hard cap 为 2,000,000 records，产品资格 ceiling 为 48 h × 10 Hz = 1,728,000 records，两者不得混称。trace compactor 每秒保留保守最坏状态并保留 exact final sample；任何有限但无法安全映射到 `Int64` 秒轴或发生 subtraction overflow 的 timestamp 必须稳定返回 `compaction_axis_out_of_range`，不能 runtime trap。
- `localization_constraints.jsonl` 按 48 h × 2 Hz 资格规模覆盖 345,600 条正式决策，parser hard cap 为 400,000、单条 64 KiB、文件 768 MiB；constraint/manual/recovery 实际 JSONL 原始行数必须分别精确等于 `captureHealth.localizationConstraintRecordCount`、`captureHealth.manualLocalizationEventCount`、`captureHealth.localizationRecoveryEventCount`。完整且身份一致的 `accepted=false` 是非致命负证据，不产生 absolute prior；`manual_event_claimed_node_not_nearest` 等记录级拒绝只排除该 prior 并降级 Result。文件 framing/count/read failure 和身份串包仍阻断。manual v2/v3 均要求最近节点、第二候选间隔和 ISO/Unix 时间交叉核对。
- Native graph 的 skeleton/factor/prior 上限统一为 4096。Swift 先解析 disposition，再解释 error；`RESOURCE_REQUIRED` 保持资源暂停语义。C ABI v5 明确携带 quality byte count、runtime ABI、graph/factor SHA、factor/publish count，并拒绝未知 prior kind；quality v3 必须作为完整 strict typed DTO 解析，与 Fast/Full path、C disposition、request 的 map/SHA/session/projection policy、C trajectory/skeleton/publish/factor counts 精确一致。quality v3 额外审计 initial-map gauge、robust-consensus prior 和节点跨度至少 30 的长程 RTAB-Map loop。`metadata.initialMapPose` 是单一显式 SE(2) gauge authority，但只有同一连通分量同时具有这种长程闭环时，才可替代三个独立 absolute prior 的发布授权；没有长程闭环仍为 `LOCAL_FRAME_ONLY`。覆盖、残差、局部形变和求解器门不放宽。factor 数只认 `solver.factor_count`，RunSummary 不得使用宽松 JSON 数字强制转换或缺失字段 fallback。
