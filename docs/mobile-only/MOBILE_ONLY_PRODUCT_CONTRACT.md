...
- 价签 observation 只有属于 verified complete burst v2，且与 burst frame 构成 exact observation/frame 一对一关系时才可进入自动通过路径；解析全程 true streaming。共享 64 KiB JSONL reader 对每行 caller body 建立独立 autorelease pool，避免 Foundation 临时对象在 200k 规模长时进程中累积；保留值仍按正常强引用生存，UTF-8/duplicate-key/nesting 严格性不得放宽或重复解析。最终定位按 exact `boundNodeID` 使用 `P_final = T_final_node * inverse(T_raw_node) * P_raw`，不再按 5 秒窗口猜节点。多帧证据融合为物理实例；货架结果同时输出 `shelf_segment_id`、`distance_from_shelf_start_cm` 与 `position_ratio`。单条坏 burst/observation 或少量未消费 frame 只降级相应价签：≥3 帧可重算则保留低置信度位置，否则保留条码和空坐标；需要时另附 `RESCAN_REQUIRED` task。
- 价签结果质量门为 `ACCEPTED / LOW_CONFIDENCE`，补扫任务独立使用 `RESCAN_REQUIRED`。recovering/weak、低 depth/view、node uncertainty、位置离散、无可靠货架候选、burst/graph/exact node/raw pose/measurement method/位置权威不足都不得删除 PriceTags；可恢复多少输出多少并标记 `LOW_CONFIDENCE`。身份串包、不可界定文件、重复 durable ID、不可证明水位和数据库/结果提交损坏仍是会话级 fatal，且必须保留原始会话。
