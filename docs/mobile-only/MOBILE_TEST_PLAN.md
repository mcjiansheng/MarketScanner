...
> 状态：**当前有效**；价签 node-local v2、统一 publication invariant 与手机性能结果证据已实现；本分支完整 host、PC 和 unsigned generic iphoneos Debug 回归已通过，QualifiedDevice 真机、设备与现场详细测试仍延期；run `31307753672` 为历史 7/8 FAIL。最后核对：2026-08-15。

> 2026-08-15 性能证据增量：扫描期 `performance_samples.jsonl`、metadata 水位、immutable snapshot/result copy、PC strict parser/CSV/summary/trend、Map Studio 性能区和 malformed raw 取证已进入当前源码。性能日志损坏只关闭 `performance_qualified`，不删除有限业务结果；GPU 明确 unavailable。当前证据为 PriorMap **330/330（374.856 s）**、Qualification **30/30**、Map Studio **141/141**、独立 Swift 主机长方法 **1/1（326.803 s）**、unsigned generic iphoneos Debug 全量编译/链接以及 1600/1280/980 浏览器布局检查 PASS；正式 Release identity 必须在 clean exact commit 上通过后才允许推送。签名真机 30 分钟/2 小时、热/低磁盘/强杀/crash/MetricKit 与现场 LiDAR 均为 NOT RUN。
