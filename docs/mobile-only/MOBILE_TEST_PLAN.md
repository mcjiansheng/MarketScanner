...
| Native/AbsolutePrior contract | 4096/4097、RESOURCE_REQUIRED+error；quality v3 / C ABI v5 strict typed DTO 的 unknown/duplicate/wrong type/Bool、path/disposition/request identity、graph/factor SHA、runtime ABI、C trajectory/skeleton/publish/factor count mismatch；unknown prior kind fail closed；单一 initial-map gauge 只有同分量长程 RTAB-Map loop 才可授权发布，无 loop 保持 LOCAL_FRAME_ONLY；RunSummary 仅投影已验证 `solver.factor_count`；constraint/manual/recovery exact raw-line watermark、345,600 资格上限、accepted=false 非致命、manual ISO/Unix 与 v2/v3 nearest/second margin | 2026-08-12 native 7,884 checks / 0 failures；完整 Swift host 1/1 PASS（1424.216 s），300,000 finalization、1,728,000 trace、400,000 tag evidence 规模通过；签名真机/现场仍 NOT RUN |
...
- 真机历史处理在核心基线 `9a93fbd0…` 复现的 committed-file/metadata/artifact hash 错误属于 snapshot transaction gate，不属于扫描质量门。macOS host 已使用真实小/中/大样本关闭 pre-open 与长哈希 `ctime` 误报，但修复版安装后仍必须使用原失败会话和一个新建简单会话各重试一次：前者不得再因 `ctime`-only drift 失败；后者若信息不足应稳定进入 `RESCAN_SESSION`，不能退回通用“处理失败”。当前真机为 NOT RUN。
