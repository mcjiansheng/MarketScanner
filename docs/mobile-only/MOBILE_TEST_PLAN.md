...
| Snapshot committed-file stable read | 真机 pre-open `ctime` 稳定化；只允许同 dev/inode/mode/link/size/mtime 且 `ctime` 向前，立即用 pathname 严格回绑 descriptor；覆盖 pre-open mtime 写入、atomic inode replacement、post-open ctime、post-open pathname replacement、hardlink、symlink 与 writable authority 拒绝 | mobile UX/source + sidecar health **41/41 PASS**；Qualification **30/30 PASS**；完整 Swift host **1/1 PASS（1270.619 s）**并实际执行 `--snapshot-stable-read-focused`；unsigned generic iphoneos Debug/Release `BUILD SUCCEEDED`，Release exact-HEAD identity verified；修复版真机 NOT RUN |
...
- 真机历史处理在核心基线 `9a93fbd0…` 复现的 `committed transaction file changed before open` 属于 snapshot transaction gate，不属于扫描质量门。修复版安装后必须使用原失败会话和一个新建简单会话各重试一次：前者不得再因 pre-open `ctime`-only drift 失败；后者若信息不足应稳定进入 `RESCAN_SESSION`，不能退回通用“处理失败”。同时保存新的 basename/identity-difference 诊断。当前为 NOT RUN。
...
15. 安装 `fix/mobile-snapshot-transaction-stable-read` 的最终 Release 到复现设备，先重试原失败历史会话，再处理一个刻意简短/空白的新会话；记录 snapshot 阶段是否通过、最终是 Result 还是 `RESCAN_SESSION`、处理耗时、内部 basename/identity 诊断以及 App 前后台切换影响。不得用 host focused PASS 替代该真机复测。
