...
> 文档状态：**当前有效**。最后核对日期：2026-08-17（version 5 身份、Debug 完整测试结果与生产资格分离合同已实现；真机与现场证据仍沿用下述已注明边界）。

## 总体

2026-08-17 构建身份合同升级为 version 5：Debug 和 Release 都生成 `MarketScannerBuildIdentity.json` 并执行同一套地图校验、ARKit/RTAB-Map、连续数据库、sidecar、finalization、处理和 Result 链路。Debug clean/dirty 均可开始扫描，identity 额外绑定实际 `source_ref` 和 `git diff --binary HEAD --` 的 SHA-256；Release 要求全部 tracked 文件 clean，clean Release 才为 `production_eligible=true`。结果合同把“可得到完整最终成果”和“具备生产资格”拆开：Debug + `CALIBRATION_PENDING` 在其他真实质量门通过时生成 `COMPLETE/TEST`、全部地图/表格/价签和 `publish_permitted=true`，同时固定 `production_publish_permitted=false`；clean Release 仍要求冻结标定后才能生产发布。该变化不放松 framing、身份、图质量、坐标、价签、穿架或原子提交门，也不把 Debug 性能或未签名构建升级为真机/现场资格证据。

当前冻结核心基线为 `core-mobile-v1@9a93fbd0ee52944eae5aebedf59ec6a08dedc934`，性能证据前序为 `feature/mobile-performance-result-evidence@eb8e71e0e7f60fbcc35bbd7bcad7278c61524604`；当前候选分支为 `fix/native-multilink-epoch-bridge`。其 `1d8964d9c351828d894cb27597e92228171bbe13` 补齐 native 多链路 epoch bridge，`e1a223f4fad22cdc279a5cf018073f0368080a86` 使 Debug/Release 进入同一完整扫描链，本轮后续提交继续收口完整测试结果、PC 对等处理和身份可复现性。最终主机回归为 PriorMap **370/370**、Qualification **32/32**、Map Studio **143/143**；PC 正式 publication 已重复验证 PRODUCTION scope，伪造或矛盾的 build metadata 只能保留为非生产结果。前序冷启动、FileProvider、buffer fail-safe、finalization 顺序、性能证据和 Map Studio 恢复能力仍在该提交历史中。真机命名交互、30 分钟/2 小时、热/低磁盘/强杀/crash/MetricKit 延迟投递、Files Provider 和现场 LiDAR 未执行，发布判断仍为 **NO-GO / NOT PRODUCTION READY**。

扫描前显示名称现已并入同一候选链路：配置页输入最多 64 个字符的可选名称，纯核心与 host 执行相同的幂等清洗，空值回退 `<store>-<floor>-MMdd-HHmm`。该值只作为可选 `scanDisplayName` 进入配置、checkpoint、metadata、scan event 和历史页显示；canonical 会话目录、数据库/sidecar、地图/追踪身份和 Result 路径保持不变。旧会话缺字段继续使用目录名，命名失败不能阻断扫描。
...
- 用户截图中的 `metadata.json changed during snapshot eligibility validation` 与 `committed artifact changed during hash: rtabmap_segment_0001.db` 仍发生在快照事务，不是扫描置信度门。快照已复制并冻结为 `0444`，但 iOS/APFS 可能在第一次长顺序读取结束后才让同一 inode 的最终 publication/chmod `ctime` 对 descriptor/path 可见；旧代码因此把相同 bytes/size/mtime 的只读大数据库误判为写入。
- artifact 哈希现在最多两遍：正常首遍完全稳定立即接受；只有 dev/inode/mode/nlink/size/mtime 全等、`ctime` 只向前且 descriptor/path 最终完全一致时，才丢弃首遍 bytes/digest 并重新打开、完整读取和哈希一次。第二遍继续变化或任何内容、mtime、inode、mode、link、size 变化仍拒绝。metadata 同一次 descriptor-stable 读取同时用于 SHA 与 strict parse，SQLite read-only validation 和 generation final sweep 只在重新完整哈希仍等于 committed manifest 时接受一次性 `ctime` 稳定。
- 真实数据实际位于 `扫描结果/0811`；仓库没有用户口述的 `0823` 目录。13 个 finalized 会话全部通过 PC parse-and-hash-once input snapshot 与新 identity gate；`SupermarketSession-20260811-094906` 缺正式 metadata 且保留 live checkpoint，继续正确拒绝。手机正式 snapshot host 对 `091950`（约 5.9 MB）、`100525`（约 36 MB）和 `103343`（约 995 MB、2284 nodes）完成 snapshot + SQLite validation + commit + revalidate；在真实 `metadata.json` 和 DB hash window 注入一次性 `ctime` 变化也通过。focused 持续 ctime/content/mtime/inode/link/symlink/mode 负例继续拒绝。
- PC 没有相同的 `ctime` 比较问题，但发现独立身份断层：同一 `map-hs.6599-20260809114530.xlsx` 的 Swift 手机包 SHA 为 `289bb787…9911`，Python PC 包 SHA 为 `dcc53c78…3a45`，而 canonical SHA 同为 `f9851a0d…5dc0`、prior-map ID 同为 `6599-f9851a0de45a`。新会话额外写完整 `priorMapCanonicalSourceSha256`；PC 优先接受 exact source SHA、exact PC package SHA 或 full canonical SHA。旧会话仅在正式 package validator、exact map/store/floor、canonical 12 位 ID 后缀及合法且 sidecar 一致的手机 package SHA 全部成立时使用显式 `legacy_cross_compiler_map_identity`，报告同时保留四类 SHA。
- 真实 PC 完整 localized processing：`091950` 16/16 节点覆盖并生成 current draft；`100525` 82/82 节点覆盖、8.896 秒、review blocker 0；`103343` 2284/2284 节点覆盖并成功越过身份/处理器，但因最大修正 2.794 m、weak/lost 707.194 秒和 36 条拒绝约束正常不发布。这证明大样本的后续失败是可解释的扫描/定位质量门，而不是程序仍把地图哈希或数据库误判为损坏。
- 当前仅有 macOS host、真实文件和 PC developer evidence；最终 Swift host executable 编译与 focused 已通过，但 unsigned iPhoneOS Debug/Release、签名真机历史重试、前后台/Files provider、LiDAR/现场矩阵尚未执行，不得声明 device PASS 或整体 GO。
...
- iOS 内嵌 `MarketScannerBuildIdentity` 已升级为 version 5；Python 生成/验证器和 Swift 读取器必须接受完全一致的 exact schema：`format`、`version`、`app_git_sha`、`native_core_sha256`、governance descriptor 的 `wave`、`branch`、`base_branch`、`base_sha`、`implementation_sha`、`validation_sha`，以及 `build_configuration`、`working_tree_state`、`source_ref`、`source_patch_sha256`、`production_eligible`。任何缺失、未知、重复或内部矛盾字段均 fail closed；合法 Debug 身份不再被扫描入口拒绝，并可生成完整 TEST Result。
