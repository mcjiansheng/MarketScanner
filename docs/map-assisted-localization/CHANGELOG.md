...
- 测试：审查整改后 `tools/Qualification/swift-host-tests/` 为 **173 断言 PASS**，`tools/SupermarketMapStudio/tests/test_mission_validation.py` 为 **33/33 PASS**，Map Studio 全量 **181/181 PASS**，Qualification **58/58 PASS**；unsigned generic iPhoneOS Debug 全量编译/链接 PASS，bundle identity 为当前 feature 分支、dirty patch digest、`production_eligible=false`。新增负例覆盖伪 SQLite/缺 sidecar、foreign mission、错误 workflow、声明路径与摘要缺失、manifest deny/unverified + live checkpoint、boundary 错类型/负 generation/错误 tracking/坏 hash、完整但非相邻的额外 boundary、幽灵扫描恢复、未知 checkpoint state/trigger/负 elapsed、U0002 重试不跳 U0003、phantom unit/缺 sidecar/重复 manifest 提交。
- 独立审查整改：PC 以稳定读取的磁盘 DB/metadata/sidecar/boundary 为权威，严格核对 mission/map package/store/floor/build/tracking identity、manifest 声明路径与 SHA、SQLite quick-check/Node/RGB-D、定位证据 health、previous hash 链及 boundary 双端类型/摘要/相邻性；不得把手机 `integrity!=verified` 或 `publishPermitted!=true` 提升为可发布。Swift checkpoint 不再对未知 state/trigger/负 elapsed 默认放行，缺 current DB/checkpoint 证据不得续采；finalization/next prepare 有显式同事务重试边并清除旧错误。store 在真实 finalized metadata、required sidecar、稳定 SHA/大小与 boundary 文件全部核对后才 create-if-absent 提交不可变 manifest，目录 fsync 失败显式返回；`ScanLiveCheckpoint` mission 字段修正为可解码 `var`，active duration 写前清洗。另修复 Xcode PBX 对象落在 section 外导致两个新增 Swift 文件被源码成员审计判为未进入生产 target 的问题。
...
**修复成功率全量回测**（`tools/PriorMap/batch_verify_pc_pipeline.py`，15 个具匹配地图包的会话，
40 min，源数据只读）：**PC 端到端 11/15（73.3%）**；排除 2 个地图包本身不合格的会话后 **11/13（84.6%）**。
失败分类：2 个地图包校验（`162937`/`181158` 天虹店，road_graph 与 elements 派生工件不一致——
即首轮记录的"旧编译器丢失派生道路拓扑"，非本轮改动）、1 个优化轨迹安全门（`093330` 邻居步长
15.34 m > 3.51 m 上限，属正确保护）、1 个时钟覆盖不足（`093303` 仅 1 条绑定，契约要求 ≥2）。
**含重复绑定会话：修复前 0/4 → 修复后 3/4（75%）**。
