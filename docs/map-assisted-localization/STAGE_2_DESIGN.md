...
> 文档状态：**当前有效（已实现设计）**。最后核对日期：2026-07-25。
...
- 结束扫描先递增 generation、停止 Vision 请求并在后台等待定位队列 2 秒有界 barrier，主线程不阻塞；定位 sidecar 写入还必须同时满足 tracking session ID 一致且会话未进入 finalizing，不能隐式创建新目录。
...
6. 保留 Top-3 非极大候选；
7. 只有候选先通过几何与 0.35 m/8° 安全门后，才以 `bestPose ⊖ rawPose` 校正变换观察多帧一致性；任何歧义、mismatch、安全拒绝或盆地切换都重置门，不能预热下一帧；
...
- 深度路径使用内缩 9×9 ROI，输出样本/内点数、比例、中值、MAD、平面残差和相机法向；前后景分层、孔洞、反射、平面不稳定或时间不匹配均拒绝。
- 快照 `age<=250 ms && versionLag=0` 才为 fresh；250–600 ms 降为 weak/pending，超过 600 ms 或版本落后强制 lost/review。
...
货架/柜台关联评分综合到边距离、射线朝向、结构法线、边内投影和遮挡。`MapShelf` 由源 width/height/yaw 定义稳定 `A/B` 面和 offset 起点；`MapTable/MapTableFeature` 的全部边以稳定 `E##` 参与，`MapPillar` 只作遮挡体。遮挡判断遍历所选楼层全部结构并使用最近射线命中；若几何最佳结构被另一结构遮挡，则不默认选择更远结构，只保留原始观测等待人工复核。
