...
| 版本化地图包、SHA-256、schema | 已实现 | converter/validator、可复现测试 |
| 道路图和有界空间索引 | 已实现 | road graph + 5 m grid，ID/查询测试 |
| dependency-free PNG 预览 | 已实现 | `render_prior_map.py`，样例视觉核对 |
...
| 道路软约束、歧义拒绝 | 已实现 | 2 Hz 后台、0.15 gain、0.25 m cap、Top-3 |
| 人工确认和审计 | 已实现 | manual JSONL + scan event |
| synthetic/trajectory replay | 已实现 | drift/noise/tracking loss/误差报告 |
| 旧会话和自由扫描兼容 | 已实现 | storage/workflow 分字段、PC regression |
| 完整业务首页五入口 | 部分实现 | 现有首页/菜单保留；新建扫描双模式已完成，独立“先验地图”首页入口尚未拆出 |
| 预定路线导入 | 未实现 | 可选增强项；道路图 synthetic 蛇形路线和回放已实现 |
| iOS 核心契约测试 | 已实现 | 自动编译运行 `PriorMapLocalizationCore.swift`，检查模式门控和 SE(2) 投影 |
