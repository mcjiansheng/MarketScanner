...
当前独立修复分支为 `codex/fix-mobile-prior-map-id-canonicalization`，基于 `f32ef7258453c6f347ed0460a4e87f0ebc035c0b`，不修改已冻结候选分支。除 RC-B19/J-04 component identity 外，第二次独立全量审查的 RC-B01…RC-B03、RC-B05…RC-B28 已完成本轮代码和本地主机自动化闭包；J-04 仍需冻结 prior-independent final-link component policy，并完成 node snapshot C ABI 与 constraint/manual evidence schema 的 breaking migration。RC-B04 exact-SHA CI、RC-B29 Replay/FAR policy freeze、RC-B30 Device Lab 也仍未关闭。当前证据包括既有 PriorMap 非超长分组 217 项、本轮相关增量 52/52、Map Studio 109/109、MapCase02 Swift/PC 正式套件 PASS，以及历史 I10 完整 Python 外层 Swift host 长方法 1/1 PASS（973.769 s）和本轮初版完整 host 1/1 PASS（1020.058 s）。这些不替代完整 `discover`、LiDAR、热/内存/后台/provider 或现场精度证据。
