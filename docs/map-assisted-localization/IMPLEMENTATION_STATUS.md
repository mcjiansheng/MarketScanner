...
2026-08-17 复核结论：P1-B～P2 **不是全部关闭**，当前只能标记为“代码子集已实现、发布与现场资格未完成”。本次修复将 shelf sidecar 升级为 v2：通道候选逐项持久化距离分、独立结构盆地分、拓扑可达性和组合分，并保存 `BOOTSTRAP/TRACKING/LOW_CONFIDENCE`、当前通道、当前货架/侧和低置信原因；货架候选只从过滤后的深度点到权威货架段的残差产生，窗口保留 sample/inlier/median/max 几何统计，两侧闭环只使用这些窗口几何统计，普通 RTAB-Map 视觉 inlier 仅保留为诊断，`optimizationMaxError` 改用其真实字段名，不再冒充 loop residual（旧 `rtab_loop_residual_m` 键在 v2 保留为 `null`）。资源策略现在真实限制参与深度残差计算的货架候选为 24→12，再把计算节拍降为 1/2、1/4；跳过节拍会清空暂存几何，不能用旧证据自证，原始 RTAB-Map 采集不受影响。跨 epoch 正向 bridge 仍缺 native 多链路共识输入，因此继续 fail-closed；当前 `bridge_evidence=[]` 只允许同 epoch 闭环。C-1/C-2/C-3 仍为 `CALIBRATION_PENDING`，手机 coordinate contract v2 上的 publication invariant v2 和 PC 发布门均新增显式阻断，待标定结果只能进入 `PARTIAL_REVIEW_REQUIRED`。以上修复消除了自证循环和待标定误发布，但不构成签名真机、LiDAR、Sam 现场或生产发布证明。
...
本轮主机证据为 PriorMap **345/345（419.709 s）**、Map Studio **143/143（10.272 s）**、Qualification **30/30（12.639 s）**、generated contracts 16 文件、Python/JavaScript 语法、patch check、native `rtabmap-prior-map-factor-graph`/`rtabmap-reprocess` 和 unsigned generic iPhoneOS Debug 全量编译/链接 PASS；Debug bundle 按合同不含正式 build identity。Swift host 压力峰值为 300,000 finalization **13,271,040 bytes**、1,728,000 trace（保留 172,801）**59,097,088 bytes**、400,000 tag evidence（接受 200,000）**793,640,960 bytes**，仍低于 768 MiB 门。真实浏览器 1280×720 启动、空状态、单楼层 Sam 诊断叠加和控制台检查通过；内置浏览器的临时 1024/390 视口覆盖未生效，因此窄屏只由现有 CSS/自动化合同覆盖，不能标为真实浏览器截图 PASS。
