...
2026-08-09 的 ESL blocker-closeout 核心 implementation I3 为 `fdcc5c87005a0128e0654eb43b1364898edd8f5d`。V3 exact-HEAD run `31276419986` 随后暴露 P0 Map Studio v1 manifest fixture 未同步新 Recovery/version/source-name 合同；生产 validator 保持严格，fixture 修复进入 I4 `ec1fe96fc676c03514e591c40226112cde30fe76`，G4 `17871d839834487c777824c062980f3322521cdb` 已将 descriptor implementation 绑定到 I4。核心实现包括 ARFrame-only camera preview、真实 ROI、bounded Vision、candidate lock、独立多帧 burst、可靠 segment+side quorum、dedicated shelf confirmation、additive localized tag v2、observation↔burst 双向 exact binding、PC manifest v3 与确认冲突 fail-closed。同步审查追加关闭了 confirmation cancel/clear TOCTOU、后台 generation data race、pre-admitted writer 被 inner finalization check 误拒、prior-map sentinel 后普通 ARFrame/Recovery 越过 drain，以及 delayed ESL audit 创建空后继 session/污染新 session：capture authority 由 coordinator 锁保护，commit 只能 claim 一次；session admission gate 与 finalization 线性化并等待 transaction/reservation；ordinary/terminal Recovery 权限分离；capture generation 冻结 exact tracking identity 且 audit 只写 active existing segment。PC 三个 transform/binding early-error 现在稳定写 `OFFLINE_ASSOCIATION_UNAVAILABLE / REVIEW_REQUIRED / rescan_required=true`，共享 manifest validator 也强制 strict version/Recovery binding、role/filename/source provenance，并拒绝 source DB hardlink 与非空 WAL/journal。

当前可执行证据包括 ESL capture focused Swift host、ESL finalization focused Swift host、source-contract、Stage-3 + localized-output-store **104/104**、exact P0 contracts **4/4**、Map Studio **106/106**、Python syntax、Swift parse 和补丁格式检查；最终冻结工作区的准确计数与审查结论以 [`reviews/CURRENT_REVIEW.md`](reviews/CURRENT_REVIEW.md) 和 `MOBILE_ONLY_V1R5_CLOSEOUT_REPORT.md` 为准。Xcode simulator 已能编译本轮 Swift module，但完整 build 仍被缺失 platform-scoped Eigen/PCL/OpenCV headers 阻断；真实 LiDAR 30 秒连续性、Vision 性能、强光/反光/多条码和现场确认矩阵仍在 [`ESL_CAPTURE_TODO.md`](ESL_CAPTURE_TODO.md)。最新独立复审为 `P0=0 / P1=0`，3 个低影响 P2 已登记 TODO。run `31276419986` 已有 P0 fixture failure，新 E4/V4 exact-HEAD 尚未形成 PASS。这些 host 证据不关闭 Apple clean build、Device Lab、Sam field、exact-SHA CI 或 J-04，整体判定不变。
