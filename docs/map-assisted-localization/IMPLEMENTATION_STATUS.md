...
本轮独立分支为 `codex/fix-mobile-map-selection-debug-start`，基于未修改的核心分支 `core-mobile-v1@36f606c1fa05e92210f0189c804dadd1b09721a1`；核心分支和冻结候选分支均未修改。UX/startup 和地图库安全专项既有复审均为 `P0=0 / P1=0`。当前新增地图优先选择、导入进度可见性和默认 Release Run 合同后，聚焦合同为 29/29 PASS；Swift 核心长方法 1/1（1233.541 s）、Swift parse、source membership/SwiftPM lock、生成合同 check、四张真实 XLSX 手机地图库 4/4、PC production validator 4/4、`git diff --check` 和当前修改的 unsigned iphoneos Debug 全量编译/链接均为 PASS。Debug build 日志明确确认身份文件仍被移除。提交后共享 `RTABMapApp` 默认 Release Run 的 unsigned generic iPhoneOS 全量编译/链接也已 PASS，并输出 `build identity verified`。`RTABMapApp-QualifiedDevice` 保持等价 Release 合同；设备安装和真实扫描仍必须在真机执行，不能用无签名 build 或 host 测试冒充。
