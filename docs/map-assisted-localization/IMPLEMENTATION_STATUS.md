...
2026-08-09 的 ESL blocker-closeout 核心 implementation I3 为 `fdcc5c87005a0128e0654eb43b1364898edd8f5d`。V3 exact-HEAD run `31276419986` 随后暴露 P0 Map Studio v1 manifest fixture 未同步新 Recovery/version/source-name 合同；fixture 修复进入 I4/G4。V4 run `31276999280@8ba2f697a8213a1bcd4bf6fb7197d155cb09b865` 为 7/8，唯一失败是 macOS/iOS host contract 的 §18 timer 固定等待抖动。当前 I5 `4d78d4646c01fe50bc0ac07eb2266879a24348db` 已由 G5 `d2eb9e2cb9b349179c410205d8eb9f44c2c30188` 绑定：真实 timer 改为有界 liveness 轮询，worker mode 不再重复默认 E2E；长回归还关闭了 mismatched Result removal tombstone 在后续恢复中重新取得 canonical authority 的 P1，异常 inode 现在永久进入 conflict namespace。ESL 核心行为、session admission/finalization、PC strict manifest/source provenance 与三类 confirmation policy 保持不变。

当前可执行证据包括 ESL capture/finalization focused、source-contract、长时 Swift host workflow **1/1（943.159 s）**、iOS/C4 **12/12**、Stage-3 + localized-output-store **104/104**、exact P0 contracts **4/4**、Map Studio **106/106**、Python syntax、Swift parse 和补丁格式检查；最终冻结工作区的准确计数与审查结论以 [`reviews/CURRENT_REVIEW.md`](reviews/CURRENT_REVIEW.md) 和 `MOBILE_ONLY_V1R5_CLOSEOUT_REPORT.md` 为准。Xcode simulator 已能编译本轮 Swift module，但本机完整 build 仍被缺失 platform-scoped Eigen/PCL/OpenCV headers 阻断；真实 LiDAR、Vision 性能和现场矩阵仍在 [`ESL_CAPTURE_TODO.md`](ESL_CAPTURE_TODO.md)。I5 第二轮独立复审为 `P0=0 / P1=0`，新增测试深化项已登记 TODO。V4 exact-HEAD 为 7/8 FAIL，新 I5/G5 仍需 E5/V5 和 committed exact-HEAD run；这些 host 证据不关闭 Apple clean build、Device Lab、Sam field、exact-SHA CI 或 J-04，整体判定不变。
