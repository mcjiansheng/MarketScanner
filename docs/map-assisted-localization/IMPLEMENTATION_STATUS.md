...
2026-08-17 复核结论：P1-B～P2 **不是全部关闭**，当前只能标记为“代码子集已实现、发布与现场资格未完成”。本次修复先将 shelf sidecar 升级为 v2：通道候选逐项持久化距离分、独立结构盆地分、拓扑可达性和组合分，并保存 `BOOTSTRAP/TRACKING/LOW_CONFIDENCE`、当前通道、当前货架/侧和低置信原因；拓扑不可达 top1 不再成为下一帧 authority。货架候选只从过滤后的深度点到权威货架段的残差产生，窗口保留实际观测节点数及 sample/inlier/median/max 几何统计，重复 node binding 不再重置窗口；两侧闭环只使用这些窗口几何统计并把 RTAB loop 两端绑定到 exact window range，普通 RTAB-Map 视觉 inlier 仅保留为诊断，`optimizationMaxError` 改用其真实字段名，不再冒充 loop residual（旧 `rtab_loop_residual_m` 键在 v2 保留为 `null`）。资源策略现在真实限制通道与货架候选为 24→12，再把计算节拍降为 1/2、1/4；动态主导判定使用实际观测节点数，跳过节拍会清空暂存几何，不能用旧证据自证，原始 RTAB-Map 采集和 free-space 淘汰不受影响。后续 pose-transition v3 已补上 native 多链路共识输入：iOS 从与 loop callback 相同的 RTAB-Map Statistics 事件冻结 exact global/local-space `Link`，绑定两端 node/map component 和采集 epoch，使用各 epoch 的 software correction 与 native node pose 推导 bridge transform；finalization 只追加一次 transition，不回写已落盘 JSONL。至少两组 node-disjoint 且与 transition transform 一致的 native link 才形成 component-scoped bridge；单链路、重复/反向 pair、共享端点、component 错绑、冲突 transform、任一相邻 transition 缺 bridge以及旧 v2 aggregate counter 都不授权跨 epoch loop。无共识 transition 仍如实保存 `bridge_evidence=[]`，同 epoch 行为不变。C-1/C-2/C-3 仍为 `CALIBRATION_PENDING`，手机 coordinate contract v2 上的 publication invariant v2 和 PC 发布门均显式阻断，待标定结果只能进入 `PARTIAL_REVIEW_REQUIRED`。当前自动化已证明合同正反例和 Swift/Python 同构，但尚未证明真机实际能稳定产生两组跨 epoch native edge；因此以上代码关闭了“无 producer”缺口，不构成签名真机、LiDAR、Sam 现场或生产发布证明。
...
本轮既有主机证据为 PriorMap **369/369（385.868 s）**、Map Studio **143/143（9.539 s）**、Qualification **30/30（13.062 s）**、generated contracts 16 文件、Python/JavaScript 语法、patch check、native `rtabmap-prior-map-factor-graph`/`rtabmap-reprocess` 和 unsigned generic iPhoneOS Debug 全量编译/链接 PASS；其中“Debug bundle 不含身份”是 version 3 历史行为，已由 2026-08-17 version 4 Debug/Release 同路合同取代。新合同已单独通过 Qualification **32/32**、扫描 UX/sidecar/yaw **61/61**、Swift parse、Python compile、generated contracts 16 文件和 patch check；unsigned generic iPhoneOS Debug 全量构建成功，包内身份经复核为 version 4、`debug`、`dirty`、`production_eligible=false`。Swift host 压力峰值为 300,000 finalization **13,303,808 bytes**、1,728,000 trace（保留 172,801）**59,179,008 bytes**、400,000 tag evidence（接受 200,000）**792,821,760 bytes**，仍低于 768 MiB 门但只保留约 12.5 MB 余量。真实浏览器 1280×720 启动、空状态、单楼层 Sam 诊断叠加和控制台检查通过；内置浏览器的临时 1024/390 视口覆盖未生效，因此窄屏只由现有 CSS/自动化合同覆盖，不能标为真实浏览器截图 PASS。
