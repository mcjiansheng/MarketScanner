...
The Sam evidence remains historical input and has not been rerun. P7R4 implementation `14f2b45fb23b9485e1065d1faf572de51dfae909` prevents a bounded intermediate Recovery step from making localization stable or auto-confirming a price tag, enforces pre/post-match wall-clock expiry, adds a 20-second automatic cooldown, preserves completion diagnostics under the correct episode, and exposes the actual localizer anchor and attempt reducer to integration tests. This is implementation evidence only; Xcode, LiDAR iPhone, same-route Recovery/occlusion runs, and Sam re-test remain deferred. Do not interpret this note as `SAM FIELD PASS`, `FIELD QUALIFICATION PASS`, or `PRODUCTION READY`.
