...
> 文档状态：**当前有效**。最后一次与源码交叉核对日期：2026-07-25。
>
> **2026-07-25 代码审查修复更新**：完成阶段一/二/三所有BLOCKER和HIGH级别问题修复，包括：
> 1. 修复SE(2)标签传播yaw旋转缺失问题，实现完整刚体变换
> 2. 货架重关联增加遮挡检查、远侧面检查、端点检查、候选唯一性检查等多重安全门
> 3. 统一人工定位事件时间基准，使用ARFrame.timestamp与Node.stamp对齐，支持最近节点绑定
> 4. 实现成果原子发布机制，临时目录写入+完整性校验+备份+原子替换，失败自动回滚
> 5. 人工编辑增加per-job互斥锁、乐观版本锁、数据库身份绑定，防止并发编辑冲突
> 6. JSONL解析增加严格模式和错误统计，关键文件损坏直接失败，不再静默跳过
> 7. SE(2)优化增加显式相对位姿残差和加权边约束，防止轨迹局部扭曲
> 8. 收紧自动发布条件，实现三级发布机制（草稿/人工复核/受控发布），默认禁止自动权威发布
> 9. 修复人工编辑字段白名单验证、报告统计准确性、绝对路径隐私清理等MEDIUM级别问题
...
- **已有地图辅助扫描（阶段一至阶段三实现）**：先把 `Element Info` XLSX 转换为带全文件完整性清单和多分辨率结构距离场的版本化地图包，在 iPhone 五步向导中选择地图、楼层、起点和朝向。手机只接受小幅、唯一且连续一致的结构修正；PC 必须先运行 RTAB‑Map 重处理，再生成先验地图派生 SE(2) 轨迹、质量报告、人工复核和最终价签导出。
...
阶段一/二整改和阶段三核心链路已有自动测试；真实 LiDAR iPhone 完整干跑、正式超市现场验收及本轮独立代码复审仍是发布前门槛。本文不把模拟指标表述为现场精度或生产批准。完整架构、格式、UI、测试和当前状态见 [docs/map-assisted-localization/](docs/map-assisted-localization/)，双模式操作、复核、备份和失败恢复见 [用户操作手册](docs/map-assisted-localization/USER_GUIDE.md)。
