...
- **已有地图辅助扫描（阶段二实现）**：先把 `Element Info` XLSX 转换为带多分辨率结构距离场的版本化地图包，在 iPhone 五步向导中选择地图、楼层、起点和朝向。扫描时以 ARKit 预测位姿，后台提取跨帧稳定深度结构并做 Top‑K 距离场匹配；只有小幅、唯一且连续一致的候选才调整地图对齐。道路仍只是弱先验。
...
阶段二已通过自动化回放与 iPhone arm64 无签名构建，但真实 LiDAR iPhone 的完整现场干跑及独立代码复审仍是发布前门槛；本文不把模拟指标表述为现场精度。完整架构、格式、UI、测试和当前状态见 [docs/map-assisted-localization/](docs/map-assisted-localization/)。
