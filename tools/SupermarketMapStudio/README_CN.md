...
2. 在 `单会话快速合并`、`单设备分阶段` 或 `多设备合并` 选择处理流程。右侧的 `2D 地图` 和 `3D 点云` 是同一份处理结果的两种预览，不是另外两个生成流程。
...
7. 在右侧切换 `2D 地图` 与 `3D 点云`：2D 视图可拖拽和滚轮缩放；3D 视图可拖拽旋转、滚轮缩放，并可开关点云/轨迹和调整点大小。
...
`单会话快速合并` 直接按会话中的分段顺序处理，适合不需要人工分组和校正的快速检查。`单设备分阶段` 会生成阶段清单，可将多个 segment 编组，并为每个阶段设置 `dx`、`dy`、`yaw` 校正，适合长距离扫描后的累积误差修正和人工复核。两种流程都会自动发现会话内的全部 `segment_*`。
...
- `preview_3d.json`：轨迹、结构点以及从 RTAB-Map 深度帧重建的抽样点云。
...
三维视图会从数据库的节点位姿、相机标定和 LiDAR 深度 PNG 重建点云，并使用 WebGL 显示。为控制大型会话的内存和交互开销，当前默认在全部可用节点中均匀抽取最多 96 个关键帧、按 8 像素步长采样，真实数据库不会被裁剪。当前结果是实际几何点云，但尚不是 RGB 纹理 mesh；高质量网格、纹理融合和毛刺清理仍需后续接入 RTAB-Map/C++ 后处理管线。
