...
if ! "$PYTHON_COMMAND" "$LAUNCH_DIR/tools/SupermarketMapStudio/server.py" --selfcheck --mode production; then
  echo "Supermarket Map Studio startup self-check failed." >&2
  echo "Keep this window open and export the output for support." >&2
  exit 1
fi

exec "$PYTHON_COMMAND" "$LAUNCH_DIR/tools/SupermarketMapStudio/server.py" --mode production "$@"
