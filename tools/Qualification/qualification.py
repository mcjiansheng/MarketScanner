...
def collect_device(plan_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    for index, run in enumerate(runs):
        ...
        evidence_runs.append({
            ...
            "sessionManifestSha256": _canonical_sha(session_manifest),
...
def evaluate_field(plan_path: Path, output_path: Path) -> dict[str, Any]:
    plan = _load_json(plan_path)
    if not isinstance(plan, dict) or plan.get("format") != FORMAT_FIELD_PLAN or plan.get("version") != 1:
    ...
    release_sha, release_size = _sha256(release_manifest)
    ...
    for index, run in enumerate(runs):
        ...
        for key, destination in (
            ...
            ("sourceSessionSha256", source_session_digests),
        ):
        ...
        for metric, threshold in tag_limits.items():
            if measured[metric] > _finite_number(thresholds.get(threshold), threshold):
                blockers.append(f"threshold_failed:{metric}")
        for key in ("inventoriesMatch", "factorGraphConverged", "singleConnectedComponent", "allGroundTruthTagsObserved", "userConfirmedTagsPreserved"):
    ...
    if len(source_session_digests) < 3:
