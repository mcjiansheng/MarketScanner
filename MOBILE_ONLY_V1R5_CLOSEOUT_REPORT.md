...
- iOS 内嵌 build identity 使用 version 3 exact schema。当前 implementation `f0ffcec4480ce04ac61f3a8aad2453e5b4b27a35` 已由 governance `dbc2f2626dbf655b916b9afe4ab24fbd812a2590` 绑定；evidence SHA由后续纯治理提交写入descriptor，当前值以descriptor为准。两个字段均为40位小写 SHA 后运行时 identity 才可 `isUsable`，但这仍不替代 exact-SHA CI。
