...
当前分段采集
  ...
  -> 打开新的临时数据库，继续扫描下一段
  -> 如果用户选择了外部保存位置，在后台复制刚保存的 segment 到外部目录
  -> 后台校验外部副本
  -> 校验成功后后台删除本地 segment 副本
```

这样避免 RTAB-Map 原生 C++ 层直接写入 iOS 文件提供器或外接盘路径导致崩溃，同时把扫描暂停窗口限制在“本地数据库落盘、sidecar 写入、新临时库打开和相机恢复”这几个必须同步完成的步骤。外部复制、校验和本地副本清理不再阻塞继续扫描。
...
- 如果选择了外部保存位置，复制和清理会在恢复扫描后后台执行。后台复制失败时，本地 segment 会保留，避免数据丢失。
